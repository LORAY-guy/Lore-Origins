...
class CharacterEditorState extends MusicBeatState
{
	...
	function addSettingsUI()
	{
		...
		var templateCharacter:FlxButton = new FlxButton(140, 50, "Load Template", function()
		{
			final _template:CharacterFile =
			{
				...
				flip_x: false,
				healthicon: 'guy',
				image: 'characters/guy',
				sing_duration: 6.1,
				scale: 3.5,
				healthbar_colors: [163, 73, 164],
				camera_position: [810, -570],
				position: [370, 530],
				vocals_file: null
			};
	...
	{
		return (name != 'bf' && !name.startsWith('bf-') && !name.endsWith('-player') && !name.endsWith('-dead') && !name.startsWith('play')) ||
