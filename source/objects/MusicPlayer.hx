...
class MusicPlayer extends FlxGroup 
{
	...
	public function new(instance:FreeplayState)
	{
		...
		songTxt = new FlxText(FlxG.width * 0.7, 5, 0, "", 36);
		songTxt.setFormat(Paths.font("ourple.ttf"), 36, FlxColor.WHITE, RIGHT);
		add(songTxt);

		timeTxt = new FlxText(xPos, songTxt.y + 60, 0, "", 36);
		timeTxt.setFormat(Paths.font("ourple.ttf"), 36, FlxColor.WHITE, RIGHT);
		add(timeTxt);

		for (i in 0...2)
		{
			var text:FlxText = new FlxText();
			text.setFormat(Paths.font('ourple.ttf'), 36, FlxColor.WHITE, CENTER);
			text.text = '^';
			if (i == 1)
				text.flipY = true;
			text.visible = false;
		...
		playbackTxt = new FlxText(FlxG.width * 0.6, 20, 0, "", 36);
		playbackTxt.setFormat(Paths.font("ourple.ttf"), 36, FlxColor.WHITE);
		add(playbackTxt);
